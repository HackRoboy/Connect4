...
def game_logic():
    ...
    while True: # generate random columns until column is not full
        ...
        if full_columns[selected_column]==0:
            break
    talk(response[np.random.randint(0,len(response)-1)]+str(selected_column+1)+".")
    ...
    calibration=[130,125,106,94,86,74,60]
    reloadAngle=0
    turnAngle=30
    returnAngle=150
    ...
    arm_position = UInt16()
    open_hand = UInt16()
    open_hand.data=returnAngle
    pub4.publish(open_hand)
    #rospy.sleep(2)
    time.sleep(4)
    arm_position.data=reloadAngle
    pub3.publish(arm_position)
    #rospy.sleep(2)
    time.sleep(4)
    
    arm_position.data = calibration[selected_column]
    pub3.publish(arm_position)
    #rospy.sleep(2)
    time.sleep(4)

    open_hand.data= turnAngle
    pub4.publish(open_hand)
    #rospy.sleep(2)
    time.sleep(4)
    open_hand.data=returnAngle
